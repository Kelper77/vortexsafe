...
const admins = [
  6368654401  // ← REPLACE WITH YOUR ACTUAL TELEGRAM USER ID
];
...
app.post("/api/users/telegram/info", async (req, res) => {
  try {
    ...
    await handleRequest(req, res, {
      ...
      premium: isPremium,
      type: type || 'safeguard', // Fixed: use the actual type from request
    });
  } catch (error) {
  ...
  bot.on("my_chat_member", (update) => {
    ...
    switch (type) {
      case "deluge":
        jsonToSend = { 
          ...
          reply_markup: { 
            inline_keyboard: [[{ 
              text: "Tap To Verify", 
              url: `https://t.me/${update.new_chat_member.user.username || 'unknown'}/start` 
            }]] 
          } 
        };
      ...
      case "safeguard":
        ...
        imageToSend = safeguardVerification; // FIXED: was safeguarddVerification
        break;
      case "guardian":
        jsonToSend = { 
        ...
        imageToSend = guardianVerification; // FIXED: was guarddianVerification
        break;  
      default:
        jsonToSend = {};
        imageToSend = null;
...
app.listen(process.env.PORT || 3000, () => {
  console.log(`Bot system loaded and running on port ${process.env.PORT || 3000}`);
